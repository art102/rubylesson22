...
DDEESSCCRRIIPPTTIIOONN
       ...
       string for bool), or _-_-_p_a_t_h, which does some path expansion (see _-_-_p_a_t_h below). If no type specifier is passed,
       no checks or transformations are performed on the value.

       When reading, the values are read from the system, global and repository local configuration files by default,
       and options _-_-_s_y_s_t_e_m, _-_-_g_l_o_b_a_l, _-_-_l_o_c_a_l and _-_-_f_i_l_e _<_f_i_l_e_n_a_m_e_> can be used to tell the command to read from only
...
OOPPTTIIOONNSS
       ...
       --get-urlmatch name URL
           ...
           given just the section as name, do so for all the keys in the section and list them.
       ...
       --int
           _g_i_t _c_o_n_f_i_g will ensure that the output is a simple decimal number. An optional value suffix of _k, _m, or _g in
           the config file will cause the value to be multiplied by 1024, 1048576, or 1073741824 prior to output.
       ...
       --get-color name [default]
           Find the color configured for name (e.g.  color.diff.new) and output it as the ANSI color escape sequence to
           the standard output. The optional default parameter is used instead, if there is no color configured for
           name.
...
CCOONNFFIIGGUURRAATTIIOONN FFIILLEE
       ...
       .git/config file in each repository is used to store the configuration for that repository, and $HOME/.gitconfig
       is used to store a per-user configuration as fallback values for the .git/config file. The file /etc/gitconfig
   ...
   SSyynnttaaxx
       ...
       and continues until the next section begins. Section names are not case sensitive. Only alphanumeric characters,
       ...
       have to be escaped as \" and \\, respectively). Section headers cannot span multiple lines. Variables may belong
       directly to a section or to a given subsection. You can have [section] if you have [section "subsection"], but
       you don’t need to.
       ...
       variables, in the form _n_a_m_e _= _v_a_l_u_e. If there is no equal sign on the line, the entire line is taken as _n_a_m_e and
       the variable is recognized as boolean "true". The variable names are case-insensitive, allow only alphanumeric
       characters and -, and must start with an alphabetic character. There can be more than one value for a given
       variable; we say then that the variable is multivalued.

       Leading and trailing whitespace in a variable value is discarded. Internal whitespace within a variable value is
       retained verbatim.
       ...
       when converting value to the canonical form using _-_-_b_o_o_l type specifier; _g_i_t _c_o_n_f_i_g will ensure that the output
       is "true" or "false".
       ...
       characters (i.e. it contains _# or _;). Double quote " and backslash \ characters in variable values must be
   ...
   VVaarriiaabblleess
       ...
       advice.*
           ...
           pushNonFFDefault
               Advice to set _p_u_s_h_._d_e_f_a_u_l_t to _u_p_s_t_r_e_a_m or _c_u_r_r_e_n_t when you ran ggiitt--ppuusshh(1) and pushed _m_a_t_c_h_i_n_g _r_e_f_s by
               default (i.e. you did not provide an explicit refspec, and no _p_u_s_h_._d_e_f_a_u_l_t configuration was set) and it
           ...
           statusHints
               ...
               shown when writing commit messages in ggiitt--ccoommmmiitt(1), and in the help message shown by ggiitt--cchheecckkoouutt(1)
           ...
           detachedHead
               Advice shown when you used ggiitt--cchheecckkoouutt(1) to move to the detach HEAD state, to instruct how to create a
       ...
       core.checkstat
           ...
           _m_i_n_i_m_a_l. Default (or explicitly _d_e_f_a_u_l_t), is to check all fields, including the sub-second part of mtime and
           ctime.
       ...
       core.autocrlf
           ...
           not be touched. Use this setting if you want to have CRLF line endings in your working directory even though
           the repository does not have normalized line endings. This variable can be set to _i_n_p_u_t, in which case no
       ...
       core.gitProxy
           ...
           The special string none can be used as the proxy command to specify that no proxy be used for a given domain
       ...
       core.ignoreStat
           ...
           is useful on systems where those are very slow, such as Microsoft Windows. See ggiitt--uuppddaattee--iinnddeexx(1). False by
           default.
       ...
       core.worktree
           ...
           variable and the _-_-_w_o_r_k_-_t_r_e_e command line option. The value can be an absolute path or relative to the path
           to the .git directory, which is either specified by --git-dir or GIT_DIR, or automatically discovered. If
           --git-dir or GIT_DIR is specified but none of --work-tree, GIT_WORK_TREE and core.worktree is specified, the
       ...
       core.deltaBaseCacheLimit
           ...
           Default is 16 MiB on all platforms. This should be reasonable for all users/operating systems, except on the
       ...
       core.commentchar
           Commands such as commit and tag that lets you edit messages consider a line that begins with this character
       ...
       core.pager
           ...
           When the LESS environment variable is unset, Git sets it to FRSX (if LESS environment variable is set, Git
           ...
           command to LESS=FRSX less -+S. The environment tells the command to set the S option to chop long lines but
           the command line resets it to the default to fold long lines.
       ...
       core.whitespace
           A comma separated list of common whitespace problems to notice.  _g_i_t _d_i_f_f will use color.diff.whitespace to
           ...
           ·   space-before-tab treats a space character that appears immediately before a tab character in the initial
           ...
           ·   tabwidth=<n> tells how many character positions a tab occupies; this is relevant for indent-with-non-tab
               and when Git fixes tab-in-indent errors. The default tab width is 8. Allowed values are 1 to 63.
       ...
       core.preloadindex
           ...
           caching semantics and thus relatively high IO latencies. With this set to _t_r_u_e, Git will do the index
       ...
       add.ignore-errors, add.ignoreErrors
           ...
           the _-_-_i_g_n_o_r_e_-_e_r_r_o_r_s option of ggiitt--aadddd(1). Older versions of Git accept only add.ignore-errors, which does
           not follow the usual naming convention for configuration variables. Newer versions of Git honor
       ...
       alias.*
           ...
           top-level directory of a repository, which may not necessarily be the current directory.  _G_I_T___P_R_E_F_I_X is set
           as returned by running _g_i_t _r_e_v_-_p_a_r_s_e _-_-_s_h_o_w_-_p_r_e_f_i_x from the original current directory. See ggiitt--rreevv--
           ppaarrssee(1).

       am.keepcr
           If true, git-am will call git-mailsplit for patches in mbox format with parameter _-_-_k_e_e_p_-_c_r. In this case
           git-mailsplit will not remove \r from lines ending with \r\n. Can be overridden by giving _-_-_n_o_-_k_e_e_p_-_c_r from
       ...
       branch.autosetuprebase
           When a new branch is created with _g_i_t _b_r_a_n_c_h or _g_i_t _c_h_e_c_k_o_u_t that tracks another branch, this variable tells
           Git to set up pull to rebase instead of merge (see "branch.<name>.rebase"). When never, rebase is never
           automatically set to true. When local, rebase is set to true for tracked branches of other local branches.
           When remote, rebase is set to true for tracked branches of remote-tracking branches. When always, rebase
           will be set to true for all tracking branches. See "branch.autosetupmerge" for details on how to set up a
       ...
       branch.<name>.remote
           ...
           push to may be overridden with remote.pushdefault (for all branches). The remote to push to, for the current
           branch, may be further overridden by branch.<name>.pushremote. If no remote is configured, or if you are not
           on any branch, it defaults to origin for fetching and remote.pushdefault for pushing. Additionally, .  (a
           period) is the current local repository (a dot-repository), see branch.<name>.merge's final note below.
       ...
       browser.<tool>.path
           Override the path for the given tool that may be used to browse HTML help (see _-_w option in ggiitt--hheellpp(1)) or
       ...
       color.branch
           A boolean to enable/disable color in the output of ggiitt--bbrraanncchh(1). May be set to always, false (or never) or
       ...
       color.diff
           ...
           This does not affect ggiitt--ffoorrmmaatt--ppaattcchh(1) nor the _g_i_t_-_d_i_f_f_-_* plumbing commands. Can be overridden on the
       ...
       color.status
           A boolean to enable/disable color in the output of ggiitt--ssttaattuuss(1). May be set to always, false (or never) or
           auto (or true), in which case colors are used only when the output is to a terminal. Defaults to false.
       ...
       color.status.<slot>
           ...
           nobranch (the color the _n_o _b_r_a_n_c_h warning is shown in, defaulting to red). The values of these variables may
       ...
       color.ui
           ...
           for the --color option. Set it to false or never if you prefer Git commands not to use color unless enabled
           explicitly with some other configuration or the --color option. Set it to always if you want all output not
           intended for machine consumption to use color, to true or auto (this is the default since Git 1.8.4) if you
       ...
       column.clean
           Specify the layout when list items in git clean -i, which always shows files and directories in columns. See
       ...
       commit.cleanup
           ...
           your log message, in which case you would do git config commit.cleanup whitespace (note that you will have
       ...
       credential.helper
           ...
           consult external storage to avoid prompting the user for the credentials. See ggiittccrreeddeennttiiaallss(7) for details.
       ...
       diff.dirstat
           ...
           lines
               ...
               of lines). This is a more expensive --dirstat behavior than the changes behavior, but it does count
           ...
           files
               ...
               the dirstat analysis. This is the computationally cheapest --dirstat behavior, since it does not have to
       ...
       diff.ignoreSubmodules
           ...
           level _d_i_f_f commands such as _g_i_t _d_i_f_f_-_f_i_l_e_s.  _g_i_t _c_h_e_c_k_o_u_t also honors this setting when reporting
           ...
           _s_t_a_t_u_s when _s_t_a_t_u_s_._s_u_b_m_o_d_u_l_e_s_u_m_m_a_r_y is set unless it is overridden by using the --ignore-submodules command
           line option. The _g_i_t _s_u_b_m_o_d_u_l_e commands are not affected by this setting.
       ...
       diff.tool
           ...
           merge.tool. The list below shows the valid built-in values. Any other value is treated as a custom diff tool
       ...
       format.thread
           ...
           letter, the --in-reply-to, and the first patch mail, in this order.  deep threading makes every mail a reply
           to the previous one. A true boolean value is the same as shallow, and a false value disables threading.

       format.signoff
           A boolean value which lets you enable the -s/--signoff option of format-patch by default.  NNoottee:: Adding the
       ...
       format.coverLetter
           ...
           be set to "auto", to generate a cover-letter only when there’s more than one patch.
       ...
       gc.rerereresolved
           Records of conflicted merge you resolved earlier are kept for this many days when _g_i_t _r_e_r_e_r_e _g_c is run. The
           default is 60 days. See ggiitt--rreerreerree(1).

       gc.rerereunresolved
           Records of conflicted merge you have not resolved are kept for this many days when _g_i_t _r_e_r_e_r_e _g_c is run. The
           default is 15 days. See ggiitt--rreerreerree(1).
       ...
       gitcvs.dbuser, gitcvs.dbpass
           Database user and password. Only useful if setting _g_i_t_c_v_s_._d_b_d_r_i_v_e_r, since SQLite has no concept of database
       ...
       grep.extendedRegexp
           If set to true, enable _-_-_e_x_t_e_n_d_e_d_-_r_e_g_e_x_p option by default. This option is ignored when the _g_r_e_p_._p_a_t_t_e_r_n_T_y_p_e
       ...
       gpg.program
           ...
           --verify $file - <$signature" is run, and the program is expected to signal a good signature by exiting with
           code 0, and to generate an ascii-armored detached signature, the standard input of "gpg -bsau $key" is fed
       ...
       gui.blamehistoryctx
           ...
           History Context menu item is invoked from _g_i_t _g_u_i _b_l_a_m_e. If this variable is set to zero, the whole history
           is shown.
       ...
       guitool.<name>.cmd
           ...
           directory, and in the environment it receives the name of the tool as _G_I_T___G_U_I_T_O_O_L, the name of the currently
       ...
       http.cookiefile
           ...
           Netscape/Mozilla cookie file format (see ccuurrll(1)). NOTE that the file specified with http.cookiefile is only
           used as input unless http.saveCookies is set.
       ...
       http.sslCAPath
           ...
           Can be overridden by the _G_I_T___S_S_L___C_A_P_A_T_H environment variable.
       ...
       http.<url>.*
           ...
           each element of the config key is compared to that of the URL, in the following order:
            ...
            2. Host/domain name (e.g., example.com in https://example.com/). This field must match exactly between the
            ...
            4. Path (e.g., repo.git in https://example.com/repo.git). The path field of the config key must match the
               ...
               Longer matches take precedence (so a config key with path foo/bar is a better match to URL path foo/bar
       ...
       interactive.singlekey
           ...
           enter). Currently this is used by the --patch mode of ggiitt--aadddd(1), ggiitt--cchheecckkoouutt(1), ggiitt--ccoommmmiitt(1), ggiitt--
           rreesseett(1), and ggiitt--ssttaasshh(1). Note that this setting is silently ignored if portable keystroke input is not
           available.

       log.abbrevCommit
           If true, makes ggiitt--lloogg(1), ggiitt--sshhooww(1), and ggiitt--wwhhaattcchhaannggeedd(1) assume --abbrev-commit. You may override this
       ...
       log.decorate
           ...
           ref name (including prefix) will be printed. This is the same as the log commands _-_-_d_e_c_o_r_a_t_e option.
       ...
       mergetool.keepBackup
           ...
           extension. If this variable is set to false then this file is not preserved. Defaults to true (i.e. keep the
           backup files).
       ...
       notes.rewriteMode
           ...
           target commit already has a note. Must be one of overwrite, concatenate, or ignore. Defaults to concatenate.
       ...
       pack.indexVersion
           ...
           *.pack file is smaller than 2 GB, however, you can use ggiitt--iinnddeexx--ppaacckk(1) on the *.pack file to regenerate
           the *.idx file.
       ...
       pretty.<name>
           ...
           would cause the invocation git log --pretty=changelog to be equivalent to running git log "--pretty=format:*
           %H %s". Note that an alias with the same name as a built-in format will be silently ignored.
       ...
       push.default
           Defines the action git push should take if no refspec is explicitly given. Different values are well-suited
           ...
           ·   nothing - do not push anything (error out) unless a refspec is explicitly given. This is primarily meant
       ...
       rebase.autostash
           When set to true, automatically create a temporary stash before the operation begins, and apply it after the
           operation ends. This means that you can run rebase on a dirty worktree. However, use with care: the final
           stash application after a successful rebase might result in non-trivial conflicts. Defaults to false.
       ...
       receive.hiderefs
           ...
           definitions to specify multiple prefix strings. A ref that are under the hierarchies listed on the value of
           this variable is excluded, and is hidden when responding to git push, and an attempt to update or delete a
       ...
       remote.<name>.mirror
           If true, pushing to this remote will automatically behave as if the --mirror option was given on the command
           line.

       remote.<name>.skipDefaultUpdate
           If true, this remote will be skipped by default when updating using ggiitt--ffeettcchh(1) or the update subcommand of
       ...
       remote.<name>.prune
           ...
           no longer exist on the remote (as if the --prune option was given on the command line). Overrides
       ...
       sendemail.<identity>.*
           Identity-specific versions of the _s_e_n_d_e_m_a_i_l_._*  parameters found below, taking precedence over those when the
           this identity is selected, through command-line or _s_e_n_d_e_m_a_i_l_._i_d_e_n_t_i_t_y.
       ...
       sendemail.to, sendemail.smtpdomain, sendemail.smtpserver, sendemail.smtpserverport, sendemail.smtpserveroption,
       ...
       status.submodulesummary
           ...
           for all submodules when diff.ignoreSubmodules is set to _a_l_l or only for those submodules where
           submodule.<name>.ignore=all. To also view the summary for ignored submodules you can either use the
       ...
       submodule.<name>.path, submodule.<name>.url, submodule.<name>.update
           The path within this project, URL, and the updating strategy for a submodule. These variables are initially
           populated by _g_i_t _s_u_b_m_o_d_u_l_e _i_n_i_t; edit them to override the URL and other values found in the .gitmodules
           file. See ggiitt--ssuubbmmoodduullee(1) and ggiittmmoodduulleess(5) for details.
       ...
       submodule.<name>.ignore
           ...
           "all", it will never be considered modified, "dirty" will ignore all changes to the submodules work tree and
       ...
       transfer.hiderefs
           This variable can be used to set both receive.hiderefs and uploadpack.hiderefs at the same time to the same
       ...
       uploadpack.hiderefs
           ...
           definitions to specify multiple prefix strings. A ref that are under the hierarchies listed on the value of
           this variable is excluded, and is hidden from git ls-remote, git fetch, etc. An attempt to fetch a hidden
       ...
       uploadpack.keepalive
           When upload-pack has started pack-objects, there may be a quiet period while pack-objects prepares the pack.
           Normally it would output progress information, but if --quiet was used for the fetch, pack-objects will
           output nothing at all until the pack data begins. Some clients and networks may consider the server to be
           hung and give up. Setting this option instructs upload-pack to send an empty keepalive packet every
           uploadpack.keepalive seconds. Setting this option to 0 disables keepalive packets entirely. The default is 5
           seconds.
...
Git 1.9.1                                              03/21/2016                                         GIT-CONFIG(1)
